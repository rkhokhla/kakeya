...
For each sample, we computed ASV signals ($\hat{D}$, $\text{coh}^\star$, $r_{\text{LZ}}$) on actual embeddings and analyzed the full-scale score distribution to assess whether ASV discriminates structural quality in real production data at scale.
...
\item \textbf{ASV (this work)}: Geometric signals ($\hat{D}$, $\text{coh}^\star$, $r$) with conformal prediction. Latency: 77ms p95; Cost: \$0.000002 per verification.
\end{itemize}
